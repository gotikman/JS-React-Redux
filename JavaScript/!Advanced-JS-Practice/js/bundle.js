...
function calc() {
  ...
  function initLocalSetting(selector, activeClass) {
    const elements = document.querySelectorAll(selector); //! Наздачаєм автоматичний вибір пунктів з LC
  ...
  function calcTotal() {
    if (!sex || !height || !weight || !age || !ratio) {
      //!перевірка на активність пунктів
  ...
  getStaticInformation('.calculating__choose_big div', 'calculating__choose-item_active'); //! функція для полів вводу, отримуєм дані
...
function forms(formSelector, modalTimerId) {
  ...
  const message = {
    // обєкт для виводу користувачу повідомлень
    loading: 'img/form/spinner.svg',
    success: 'Дякую! Незабаром ми з Вами звяжимось',
    failure: 'Упс! Щось пішло не так...'
  };
  forms.forEach(item => {
    // Вішаєм функцію на усі форми
    bindPostData(item);
  });

  function bindPostData(form) {
    // функція що будем викликати при відправці форми
    form.addEventListener('submit', evt => {
      // вішаєм подію - відправка форми
      evt.preventDefault();
      ...
      statusMessage.style.cssText = `
                ...
                margin: 0 auto;
                `;
      form.insertAdjacentElement('afterend', statusMessage); //! створюєм обєкт FD в який буде приходити форма що передали

      const formData = new FormData(form); //! FormData конвертуєм в JSON
      ...
      (0,_services_services__WEBPACK_IMPORTED_MODULE_1__.postData)('http://localhost:3000/requests', JSON.stringify(object)).then(data => {
        //! код у позитивному випадку
        console.log(data);
        showThanksModal(message.success);
        statusMessage.remove(); // видаляєм повідомлення
      }).catch(() => {
        //! код у випадку помилки
        showThanksModal(message.failure);
      }).finally(() => {
        //! код у будь-кому випадку
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  modalButtons.forEach(btn => {
    // показуєм модальне вікно при кліці на кнопку
  ...
  modal.addEventListener('click', evt => {
    // закриваєм модальне вікно по кліці за межами і хрестик
    if (evt.target === modal || evt.target.getAttribute('data-close') == '') {
      closeModal(modalSelector);
    }
  });
  document.addEventListener('keydown', evt => {
    // закриваєм по кнопці Escape
    if (evt.code === "Escape" && modal.classList.contains('show')) {
      // перевірка щоб не спрацьов. пості.    
      closeModal(modalSelector);
    }
  });

  function showModalByScroll() {
    // відкриття після прокрутки до кінця
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  hideTabContent();
  showTabContent(); //! вішаєм делегування подій

  tabsParent.addEventListener('click', evt => {
    const target = evt.target; // створюєм щоб легше юзати надалі

    if (target && target.classList.contains(tabsSelector.slice(1))) {
      //перевіряєм кнопку, вирізаєм "."
      tabs.forEach((item, i) => {
        // перебираєм і шукаєм номер що клацнули
        if (target == item) {
          //перевірка табнутого з перебираємим
          hideTabContent();
          showTabContent(i);
        }
      });
...
function timer(id, deadLine) {
  ...
  function getTimeRemaining(endtime) {
    const t = Date.parse(endtime) - Date.parse(new Date()),
          // отримуєм загальну різницю в мілісек.
    days = Math.floor(t / (1000 * 60 * 60 * 24)),
          // отримуєм дні
    hours = Math.floor(t / (1000 * 60 * 60) % 24),
          // отримуєм години, залишок після .
    minutes = Math.floor(t / 1000 / 60 % 60),
          // отримуєм хвилини, залишок після .
...
const postData = async (url, data) => {
  const res = await fetch(url, {
    // чекаєм і отримуєм promise дані по запросу
    method: "POST",
    headers: {
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2021-12-19');
