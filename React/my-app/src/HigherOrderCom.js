...
const withSlider = (BaseComponent, getData) => {     //* with - поч для хуків
    return (props) => {
        ...
        return <BaseComponent
            {...props}                         //? Розгортаєм props отримані вище
            slide={slide}
            autoplay={autoplay}
...
const SliderWithSecontFetch = withSlider(SliderSecond, getDataFromSecondFetch);
// ----------------------------------------------------------------------------

const withLogger = WrappedComponent => props => {       //! HOC
    useEffect(() => {
...
const Hello = () => {                                 //! звич комп.
    return (
        <h1>Hello</h1>
    )
}

const HelloWithLogger = withLogger(Hello)            //! передаєм в HOC
// ----------------------------------------------------------------------------
